...
```{r}
#interactions_all = as.data.frame(read.table("RegBuild_BLUEPRINT_annotations_all_valid_chromatin_status.txt", header = TRUE, sep = " "))
#head(interactions_all)
#write.table(filter(interactions_all, CellType=="Mac0")[,c(1:16,21)],"RegBuild_BLUEPRINT_annotations_Mac0_with_fragment_activity_scores.txt",
#            row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE, append = FALSE)
...
p_BaitActive = sum(interactions$BaitActive) / nrow(interactions)
p_BaitActive
```
...
p_OtherEndActive = sum(interactions$OtherEndActive) / nrow(interactions)
p_OtherEndActive
```
...
for (i in 1:10000) {
  ...
  # write code here
  em_perm[i] = log2( sum(shuffled_data$BaitActive & shuffled_data$OtherEndActive) / ( p_BaitActive * p_OtherEndActive * n_total ) )
}
```
...
```{r}
# write your code here
enrichment_matrix[1,1] > quantile(em_perm, probs = 0.95)
```